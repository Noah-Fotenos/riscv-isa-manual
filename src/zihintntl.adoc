...
instruction") exhibit poor temporal locality of reference. [#norm:zihintntl:no_arch_state_change]#The NTL
...
[#norm:zihintntl:NTL.P1]#The NTL.P1 instruction indicates that the target instruction does not
...
[#norm:zihintntl:NTL.PALL]#The NTL.PALL instruction indicates that the target instruction does not
...
[#norm:zihintntl:NTL.S1]#The NTL.S1 instruction indicates that the target instruction does not
exhibit temporal locality within the capacity of the innermost level of
shared cache in the memory hierarchy. NTL.S1 is encoded as
ADD _x0, x0, x4_.#

[#norm:zihintntl:NTL.ALL]#The NTL.ALL instruction indicates that the target instruction does not
...
[#norm:zihintntl:compressed_varients]#If the C or Zca extension is provided, compressed variants of these HINTs are
...
[#norm:zihintntl:inst_affect_range]#The NTL instructions affect all memory-access instructions except the
...
[#norm:zihintntl:NTL_Zicob_prefetch_outer]#When an NTL instruction is applied to a prefetch hint in the Zicbop
