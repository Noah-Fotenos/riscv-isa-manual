...
private cache in the memory hierarchy.# [#norm:NTL.P1_en]#NTL.P1 is encoded as
ADD _x0, x0, x2_.#
...
cache in the memory hierarchy.# [#norm:NTL.PALL_en]#NTL.PALL is encoded as ADD _x0, x0, x3_.#

[#norm:NTL.S1_op]#The NTL.S1 instruction indicates that the target instruction does not
exhibit temporal locality within the capacity of the innermost level of
shared cache in the memory hierarchy.# [#norm:NTL.S1_en]#NTL.S1 is encoded as
ADD _x0, x0, x4_.#
...
the memory hierarchy.# [#norm:NTL.ALL_en]#NTL.ALL is encoded as ADD _x0, x0, x5_.#
...
[#norm:NTL-compressed_varients]#If the C or Zca extension is provided, compressed variants of these HINTs are
