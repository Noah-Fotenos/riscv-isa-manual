...
[#norm:Ssctr:mctrctl-sz_acc_op]#The `mctrctl` register is a 64-bit read/write register that enables and configures the CTR capability.#
...
|===
|Field |Description
|M, S, U |[#norm:mctrctl-mode_en]#Enable transfer recording in the selected privileged mode(s).#

|RASEMU |[#norm:mctrctl-rasemu]#Enables RAS (Return Address Stack) Emulation Mode.#  See <<RAS (Return Address Stack) Emulation Mode>>.

|MTE |[#norm:mctrctl-mte]#Enables recording of traps to M-mode when M=0.#  See <<External Traps>>.

|STE |[#norm:mctrctl-ste]#Enables recording of traps to S-mode when S=0.#  See <<External Traps>>.

|BPFRZ |[#norm:mctrctl-bpfrz]#Set `sctrstatus`.FROZEN on a breakpoint exception that traps to M-mode or S-mode.# See <<Freeze>>.

|LCOFIFRZ |[#norm:mctrctl-lcofifrz]#Set `sctrstatus`.FROZEN on local-counter-overflow interrupt (LCOFI) that traps to M-mode or S-mode.# See <<Freeze>>.

|EXCINH |[#norm:mctrctl-excinh]#Inhibit recording of exceptions.#  See <<Transfer Type Filtering>>.

|INTRINH |[#norm:mctrctl-intrinh]#Inhibit recording of interrupts.#  See <<Transfer Type Filtering>>.

|TRETINH |[#norm:mctrctl-tretinh]#Inhibit recording of trap returns.#  See <<Transfer Type Filtering>>.

|NTBREN |[#norm:mctrctl-ntbren]#Enable recording of not-taken branches.#  See <<Transfer Type Filtering>>.

|TKBRINH |[#norm:mctrctl-tkbrinh]#Inhibit recording of taken branches.#  See <<Transfer Type Filtering>>.

|INDCALLINH |[#norm:mctrctl-indcallinh]#Inhibit recording of indirect calls.#  See <<Transfer Type Filtering>>.

|DIRCALLINH |[#norm:mctrctl-dircallinh]#IInhibit recording of direct calls.#  See <<Transfer Type Filtering>>.

|INDJMPINH |[#norm:mctrctl-indjmpinh]#Inhibit recording of indirect jumps (without linkage).#  See <<Transfer Type Filtering>>.

|DIRJMPINH |[#norm:mctrctl-dirjmpinh]#Inhibit recording of direct jumps (without linkage).#  See <<Transfer Type Filtering>>.

|CORSWAPINH |[#norm:mctrctl-corswapinh]#Inhibit recording of co-routine swaps.#  See <<Transfer Type Filtering>>.

|RETINH |[#norm:mctrctl-retinh]#Inhibit recording of function returns.#  See <<Transfer Type Filtering>>.

|INDLJMPINH |[#norm:mctrctl-indljmpinh]#Inhibit recording of other indirect jumps (with linkage).#  See <<Transfer Type Filtering>>.

|DIRLJMPINH |[#norm:mctrctl-dirljmpinh]#Inhibit recording of other direct jumps (with linkage).#  See <<Transfer Type Filtering>>.
|Custom[3:0] | [#norm:mctrctl-custom]#WARL bits designated for custom use.#  The value 0 must correspond to standard behavior.  See <<Custom Extensions>>.
|===
...
[#norm:Ssctr:sctrctl_op]#The `sctrctl` register provides supervisor mode access to a subset of `mctrctl`.#

[#norm:Ssctr:sctrctl_acc]#Bits 2 and 9 in `sctrctl` are read-only 0. As a result, the M and MTE fields in `mctrctl` are not accessible through `sctrctl`.  All other `mctrctl` fields are accessible through `sctrctl`.#

==== Virtual Supervisor Control Transfer Records Control Register (`vsctrctl`)

[#norm:Ssctr:vsctrctl_sz_acc_op]#If the H extension is implemented, the `vsctrctl` register is a 64-bit read/write register that is VS-mode's version of supervisor register `sctrctl`.  When V=1, `vsctrctl` substitutes for the usual `sctrctl`, so instructions that normally read or modify `sctrctl` actually access `vsctrctl` instead.#
...
|===
|Field |Description
|S |[#norm:vsctr-s]#Enable transfer recording in VS-mode.#
|U |[#norm:vsctrctl-u]#Enable transfer recording in VU-mode.#
|STE |[#norm:vsctrctl-ste]#Enables recording of traps to VS-mode when S=0.#  See <<External Traps>>.
|BPFRZ |[#norm:vsctrctl-bpfrz]#Set `sctrstatus`.FROZEN on a breakpoint exception that traps to VS-mode.# See <<Freeze>>.
|LCOFIFRZ |[#norm:vsctrctl-lcofifrz]#Set `sctrstatus`.FROZEN on local-counter-overflow interrupt (LCOFI) that traps to VS-mode.# See <<Freeze>>.
...
|===
|Field |Description
|DEPTH |[#norm:sctrdepth-depth]#WARL field that selects the depth of the CTR buffer.# Encodings:
...
[#norm:Ssctr:sctrstatus_acc]#Undefined bits in `sctrstatus` are WPRI. Status fields may be added by future extensions,
...
[#norm:Ssctr:ctrsource_sz_acc_op]#`ctrsource` is an MXLEN-bit WARL register that must be able to hold all valid virtual or physical addresses that can serve as a `pc`. It need not be able to hold any invalid addresses; implementations may convert an invalid address into a valid address that the register is capable of holding.  When XLEN < MXLEN, both explicit writes (by software) and implicit writes (for recorded transfers) will be zero-extended.#
...
| 6 | [#norm:ttype6]#_reserved_#
| 7 | [#norm:ttype7]#_reserved_#
